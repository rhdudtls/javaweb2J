...
public class MemberController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(com.equals("/MemberPwdRemake")) {
			viewPage += "/memberPwdRemake.jsp";
		}
		else if(com.equals("/MemberPwdRemakeOk")) {
			command = new MemberPwdRemakeOkCommand();
			command.execute(request, response);
			viewPage = "/include/message.jsp";
		}
		else if(level > 1) {	// 비회원인경우는 초기화면으로 보내버린다.
		...
		else if(com.equals("/MemberUpdatePwdOk")) {
			command = new MemberUpdatePwdOkCommand();
			command.execute(request, response);
			viewPage = "/include/message.jsp";
		}
