...
<head>
	...
	<script>
		...
		function ticketDelete() {
			let ans = confirm("해당 예매를 취소하시겠습니까?");
			if(!ans) return false;
			location.href="${ctp}/TicketDelete.ti?&pag=${pag}&pageSize=${pageSize}&idx=${tvo.idx}";
		}
	</script>
</head>
<body>
...
<p><br/></p>
<div class="container">
  <h2 class="h1">예매 내역 확인</h2>
  <div class="card">
    <div class="card-body">
		<div class="row">
			<div class="col-sm-4"><img src="${ctp}/assets/img/content/${cvo.photo}" width="250px"/></div>
			<div class="col-sm-8">
		    	<table class="table table-borderless text-center">
		    		<tr>
		    			<td colspan="10"><font size="5" color="orange">👇예매 내역👇</font></td>
		    		</tr>
		    		<tr>
		    			<th>공연 제목:</th>
						<td>${cvo.title}</td>
					...
					<tr>
						<th>예매 날짜:</th>
						<td>${tvo.ticketDate}</td>
						<th>예매 좌석:</th>
						<td>${tvo.ticketSeat}</td>
		    		</tr>
		    		<tr>
						<th>총 가격:</th>
						<td>${tvo.totalPrice}원</td>
						<th>티켓 구매 시간:</th>
						<td>${fn:substring(tvo.buyTicketTime,0,19)}</td>
					</tr>
		    	</table>
			</div>
		</div>
		<div class="text-right">
			<input type=button value="예매취소" onclick="ticketDelete()" class="btn btn-danger mr-2"/>
			<input type=button value="돌아가기" onclick="location.href='${ctp}/TicketList.ti?&pag=${pag}&pageSize=${pageSize}';" class="btn btn-secondary"/>
		</div>
    </div>
  </div>
</div>
