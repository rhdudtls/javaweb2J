...
<p><br/></p>
<div class="container">
  <h2 class="text-center h1">교환글리스트</h2>
  <h3 class="text-center"><font color="orange">🚫티켓의 판매 행위는 절대적으로 금지합니다. 발각될 경우 탈퇴조취됩니다.🚫</font></h3>
  <table class="table table-borderless">
    <tr>
      <c:if test="${sLevel <= 1}"><td><input type="button" value="글작성" onclick="location.href='${ctp}/ChangeInput.ch';" class="btn btn-success"/></td></c:if>
      <td class="text-right">
        <!-- 한페이지 분량처리 -->
  ...
  <div class="text-center m-4">
	  <ul class="pagination justify-content-center pagination-sm">
	    <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeList.ch?pageSize=${pageSize}&pag=1">첫페이지</a></li></c:if>
	    <c:if test="${curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeList.ch?pageSize=${pageSize}&pag=${(curBlock-1)*blockSize + 1}">이전블록</a></li></c:if>
	    ...
	    <c:if test="${curBlock < lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeList.ch?pageSize=${pageSize}&pag=${(curBlock+1)*blockSize + 1}">다음블록</a></li></c:if>
	    <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeList.ch?pageSize=${pageSize}&pag=${totPage}">마지막페이지</a></li></c:if>
	  </ul>
  </div>
  ...
  <div class="container text-center">
    <form name="searchForm" method="post" action="${ctp}/ChangeSearch.ch">
      ...
      <select name="search">
        ...
        <option value="content">공연&좌석정보</option>
      </select>
