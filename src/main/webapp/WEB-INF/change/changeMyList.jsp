...
<head>
	...
	<script>
		'use strict';
		if(${pag} > ${totPage}) location.href="${ctp}/ChangeMyList.ch?pag=${totPage}&pageSize=${pageSize}";
		function pageCheck(){
	    	let pageSize = document.getElementById("pageSize").value;
	    	location.href = "${ctp}/ChangeMyList.ch?pag=${pag}&pageSize="+pageSize;
		}
	</script>
</head>
<body>
...
<p><br/></p>
<div class="container">
  <h2 class="text-center h1">내가쓴교환글</h2>
    ...
    <c:forEach var="vo" items="${vos}" varStatus="st">
      <tr>
        ...
        <td class="text-center">
          <a href="${ctp}/ChangeInfo.ch?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}&flag=myChange">${vo.title}</a>
  ...
  <div class="text-center m-4">
	  <ul class="pagination justify-content-center pagination-sm">
	    <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=1">첫페이지</a></li></c:if>
	    <c:if test="${curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=${(curBlock-1)*blockSize + 1}">이전블록</a></li></c:if>
	    <c:forEach var="i" begin="${curBlock*blockSize + 1}" end="${curBlock*blockSize + blockSize}" varStatus="st">
	      <c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link text-white bg-secondary border-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=${i}">${i}</a></li></c:if>
	      <c:if test="${i <= totPage && i != pag}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=${i}">${i}</a></li></c:if>
	    </c:forEach>
	    <c:if test="${curBlock < lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=${(curBlock+1)*blockSize + 1}">다음블록</a></li></c:if>
	    <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ChangeMyList.ch?pageSize=${pageSize}&pag=${totPage}">마지막페이지</a></li></c:if>
	  </ul>
  </div>
</div>
<p><br/></p>
